...
- What changed: Extended the editor with the earlier UX slice, then corrected review exceptions so all Mega forms reuse their explicitly mapped regular form's usage data, `gallademega` is ignored, and Mega Stones transform only their explicit compatible base form.
- Verification: `pnpm verify:deploy` passes with form/localization audits, 17 test files / 125 tests, production build, and 10 rendered-output/API checks. New whole-catalog Mega inheritance, `gallademega` rejection, and regional-form regressions pass; the live audit passes for 358 forms.
