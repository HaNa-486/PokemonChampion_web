...
- Complete-review batch at `21bd246`: (1) reconcile deleted EntryActions target to Untagged and reject invalid duplicate store targets; verify mounted-source deletion then visible fallback duplicate and no mutation for invalid IDs; (2) retain positional blank move slots in scrapbook editing, clearing, migration, and persistence using empty-string placeholders in the existing string array; compact only on explicit team submission and preserve team-mode behavior; verify slot 4 first, slot 2 set/clear, reopen and migration round-trip; (3) single-flight scrapbook hydration, block mutations until successful load, show loading/retry UI in scrapbook/add surfaces, and preserve stored data on load failure; verify deferred reads, rejected early mutations, existing data retention and retry. These local reversible P2 correctness/data-loss repairs are one serial batch across ScrapbookView, AddToScrapbookDialog, ChampionsApp, scrapbook-store and their tests. No database migration, data reset, policy change, or production action. Re-run exact deployment checks and independent whole-artifact review before owner-only Dev and fresh UAT.
- Additional material finding at candidate `083a3ad`: nested move reverse lookup receives a zero-argument callback bound to the source detail Pokémon, so selecting another result adds the wrong Pokémon. Fix the Detail dialog/panel callback to accept `Pokemon`, forward the unbound parent callbacks in DatabaseViews and ScrapbookView, and explicitly pass the source Pokémon only from the detail header button. Acceptance: both database-detail and inline-scrapbook reverse lookups add distinct selected Pokémon B (modal identity and stored pokemonId), while header still adds source A and layered Escape remains correct. This is a local reversible P2 correctness fix, independently reviewed before release; no data migration or unrelated behavior change.
