...
Selecting a comparison row expands Pokémon intelligence inline without repeating the Pokémon's base-stat strip already superseded by the configured final-stat comparison above. Both this inline view and the Pokémon DB detail modal present learnable moves through the Move DB table language: sortable Move, Type, Class, Power, Accuracy, PP, Priority, Target, Effect, and usable-Pokémon-count columns. Properties/feature tags and their filter remain exclusive to Move DB. Numeric columns begin with descending order on first activation. Before a user activates a column sort, scrapbook moves retain own-type-first order, remaining types use canonical catalog order, and each type uses Physical → Special → Status order. The table is height-bounded with vertical and horizontal scrolling. The current-season move, held-item, ability, nature, AP-spread, and teammate sections each have an independent visibility toggle. Quick-finder type and form option labels follow the active locale without changing their stored/filter values. The page must remain operable and unclipped at 390, 768, and 1440 CSS pixels.
