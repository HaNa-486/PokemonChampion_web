...
type View = "pokemon" | "moves" | "abilities" | "items" | "speed" | "types";
...
const labels = {
  en: { pokemon: "Pokémon DB", moves: "Move DB", abilities: "Ability DB", items: "Held Item DB", speed: "Speed Compare", types: "Type Chart", search: "Search Pokémon or type…", current: "Regulation M-4 · Current", add: "Build & add", save: "Save changes", edit: "Edit", team: "Selected team", empty: "Choose a Pokémon to start building.", data: "Battle data updated", stale: "cached snapshot", doubles: "Doubles", singles: "Singles" },
  "zh-Hant": { pokemon: "寶可夢資料庫", moves: "招式資料庫", abilities: "特性資料庫", items: "持有物資料庫", speed: "速度比較", types: "屬性相剋", search: "搜尋寶可夢或屬性…", current: "規則 M-4 · 當前", add: "配置並加入", save: "儲存變更", edit: "編輯", team: "已選隊伍", empty: "選擇一隻寶可夢開始配置。", data: "對戰資料更新", stale: "快取資料", doubles: "雙打", singles: "單打" },
};
...
export function ChampionsApp() {
  ...
  return <div className="app-shell"><header className="topbar"><a className="brand" href="#top" aria-label="Champions Lab home"><span className="brand-mark">CL</span><span><b>CHAMPIONS LAB</b><small>Battle intelligence, built clearly.</small></span></a><nav>{(["pokemon","moves","abilities","items","types","speed"] as View[]).map((entry) => <button key={entry} className={view === entry ? "active" : ""} onClick={() => setView(entry)}>{copy[entry]}</button>)}</nav><div className="header-actions"><span className="regulation-dot">● {copy.current}</span><div className="segmented" role="group" aria-label="Team mode"><button className={format === "singles" ? "active" : ""} onClick={() => setFormat("singles")}>{copy.singles}</button><button className={format === "doubles" ? "active" : ""} onClick={() => setFormat("doubles")}>{copy.doubles}</button></div><button className="locale-button" onClick={() => setLocale(locale === "en" ? "zh-Hant" : "en")}>{locale === "en" ? "繁中" : "EN"}</button></div></header><main id="top"><PokemonTableV2 active={view === "pokemon"} locale={locale} format={format} onSelect={startNewBuild} /><MoveDatabaseV2 active={view === "moves"} locale={locale} /><ResourceDatabaseV2 active={view === "abilities"} kind="abilities" locale={locale} /><ResourceDatabaseV2 active={view === "items"} kind="items" locale={locale} />{view === "types" && <TypeChart locale={locale} />}<SpeedCompareV2 active={view === "speed"} locale={locale} format={format} /></main><footer><span>Unofficial community tool.</span><a href="https://championsbattledata.com/">Battle data provided by Pokémon Champions Battle Data</a><a href="https://github.com/smogon/pokemon-showdown">Move mechanics provided by Pokémon Showdown</a><a href="https://github.com/PokeAPI/sprites">Held-item sprites provided by PokeAPI sprites</a><DataFreshness locale={locale} /></footer><TeamTray locale={locale} format={format} onFormatChange={setFormat} onEdit={editTeamMember} /><BuildEditor selected={selected} editingMember={editingMember} locale={locale} format={format} onFormatChange={setFormat} onClose={closeEditor} /></div>;
}
