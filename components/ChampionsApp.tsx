...
import { abilities, abilityById, itemById, items, moveById, moves, pokemon, pokemonById } from "../lib/catalog";
...
function BuildEditorContent({ selected, locale, onClose }: { selected: Pokemon; locale: Locale; onClose: () => void }) {
  ...
  const submit = () => {
    ...
    const issues = validateTeam([...members, candidate], pokemonById, new Set(items.map((item) => item.id)));
