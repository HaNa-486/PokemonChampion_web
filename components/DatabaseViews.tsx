...
import { abilities, abilityById, items, moves, pokemon, pokemonByAbilityId, pokemonByMoveId } from "../lib/catalog";
...
export function PokemonTableV2({ locale, format, onSelect }: { locale: Locale; format: "singles" | "doubles"; onSelect: (entry: Pokemon) => void }) {
  ...
  const copy = locale === "zh-Hant" ? { title: "寶可夢資料庫", search: "搜尋寶可夢或屬性…", legal: "個合法型態", showing: "顯示", hint: "點擊名稱查看可學招式、特性及當季單打／雙打資料" } : { title: "Pokémon DB", search: "Search Pokémon or type…", legal: "legal forms", showing: "showing", hint: "Select a name for moves, abilities, and current Singles / Doubles data" };
  return <><section className="panel catalog-panel"><div className="panel-head"><div><p className="eyebrow">CURRENT REGULATION</p><h1>{copy.title}</h1><p>{matches.length} {copy.legal} · {copy.showing} {rows.length} · {copy.hint}</p></div><label className="search-field"><span className="sr-only">Search Pokémon</span><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder={copy.search} /></label></div><div className="table-scroll"><table><thead><tr><SortHeader label="Pokémon" column="name" sort={sort} onChange={setSort} /><SortHeader label="Type" column="type" sort={sort} onChange={setSort} />{statKeys.map((key) => <SortHeader key={key} label={statLabels[key]} column={key} sort={sort} onChange={setSort} />)}<SortHeader label="Ability" column="ability" sort={sort} onChange={setSort} /><th><span className="sr-only">Action</span></th></tr></thead><tbody>{rows.map((entry) => <tr key={entry.id}><td><button className="pokemon-name pokemon-detail-trigger" onClick={() => setDetails(entry)}><img src={entry.imageUrl} alt="" width="54" height="54" loading="lazy" /><span><strong>{localName(entry, locale)}</strong>{entry.isMega && <small>Mega</small>}</span></button></td><td><div className="badge-row">{entry.types.map((type) => <TypeBadge key={type} type={type} />)}</div></td>{statKeys.map((key) => <td key={key} className={key === "speed" ? "stat-accent" : "stat-cell"}>{entry.baseStats[key]}</td>)}<td><div className="ability-list">{entry.abilityIds.length ? entry.abilityIds.map((id) => <AbilityTooltip id={id} locale={locale} key={id} />) : "—"}</div></td><td><button className="add-button" onClick={() => onSelect(entry)} aria-label={`Configure ${entry.name}`}>+</button></td></tr>)}</tbody></table></div></section>{details && <PokemonDetailDialog key={details.id} pokemon={details} locale={locale} initialFormat={format} onClose={() => setDetails(null)} onBuild={() => { const entry = details; setDetails(null); onSelect(entry); }} />}</>;
}
