...
function PokemonUsersDialog({ selection, locale, onClose, onScrapbook }: { selection: ReverseSelection; locale: Locale; onClose: () => void; onScrapbook: (entry: Pokemon) => void }) {
...
export function PokemonTableV2({ locale, format, onSelect, onScrapbook = () => undefined, active = true }: { locale: Locale; format: "singles" | "doubles"; onSelect: (entry: Pokemon) => void; onScrapbook?: (entry: Pokemon) => void; active?: boolean }) {
  const [query, setQuery] = useState("");
  ...
  return <><section className="panel catalog-panel"><div className="panel-head"><div><p className="eyebrow">CURRENT REGULATION</p><h1>{copy.title}</h1><p>{matches.length} {copy.legal} · {copy.showing} {rangeStart}–{rangeEnd} {copy.of} {sorted.length} · {copy.hint}</p></div></div>
    ...
    <div className="table-scroll"><table><thead><tr><SortHeader label="Pokémon" column="name" sort={sort} onChange={changeSort} /><SortHeader label={localizedTerm("Type", locale)} column="type" sort={sort} onChange={changeSort} />{statKeys.map((key) => <SortHeader key={key} label={statLabels[key]} column={key} sort={sort} onChange={changeSort} initialDirection="desc" />)}<SortHeader label="TOT" column="total" sort={sort} onChange={changeSort} initialDirection="desc" /><SortHeader label={localizedTerm("Ability", locale)} column="ability" sort={sort} onChange={changeSort} /><th><span className="sr-only">Action</span></th></tr></thead><tbody>{rows.map((entry) => <tr key={entry.id}><td><button className="pokemon-name pokemon-detail-trigger" onClick={() => setDetails(entry)}><img src={entry.imageUrl} alt="" width="54" height="54" loading="lazy" /><span><strong>{localName(entry, locale)}</strong>{entry.isMega && <small>Mega</small>}</span></button></td><td><div className="badge-row">{entry.types.map((type) => <TypeBadge key={type} type={type} locale={locale} />)}</div></td>{statKeys.map((key) => <td key={key} className={key === "speed" ? "stat-accent" : "stat-cell"}>{entry.baseStats[key]}</td>)}<td className="stat-accent stat-total" data-stat="total">{totalBaseStats(entry.baseStats)}</td><td><div className="ability-list">{entry.abilityIds.length ? entry.abilityIds.map((id) => <AbilityTooltip id={id} locale={locale} key={id} />) : "—"}</div></td><td><div className="catalog-row-actions"><button className="scrapbook-add-button" onClick={() => onScrapbook(entry)} aria-label={`${locale === "zh-Hant" ? "加入畫本" : "Add to scrapbook"} ${localName(entry, locale)}`}>▣＋</button><button className="add-button" onClick={() => onSelect(entry)} aria-label={`Configure ${entry.name}`}>+</button></div></td></tr>)}</tbody></table></div><nav className="pagination" aria-label="Pokémon pages"><button disabled={safePage === 1} onClick={() => setPage((current) => Math.max(1, current - 1))} aria-label="Previous page">‹ {copy.previous}</button><span>{copy.page} <b>{safePage}</b> / {pageCount}</span><button disabled={safePage === pageCount} onClick={() => setPage((current) => Math.min(pageCount, current + 1))} aria-label="Next page">{copy.next} ›</button></nav></section>{details && <PokemonDetailDialog key={details.id} pokemon={details} locale={locale} initialFormat={format} onClose={() => setDetails(null)} onBuild={() => { const entry = details; setDetails(null); onSelect(entry); }} onScrapbook={() => onScrapbook(details)} />}</>;
}

type MoveSortKey = "name" | "type" | "category" | "power" | "accuracy" | "pp" | "priority" | "target" | "flags" | "users";

export function MoveDatabaseV2({ locale, onScrapbook = () => undefined, active = true }: { locale: Locale; onScrapbook?: (entry: Pokemon) => void; active?: boolean }) {
  const [query, setQuery] = useState("");
  ...
  return <>
    <section className="panel">
      ...
      {selected && <PokemonUsersDialog key={`${selected.kind}-${selected.id}`} selection={selected} locale={locale} onClose={() => setSelected(null)} onScrapbook={onScrapbook} />}
  </>;
}

type AbilitySortKey = "name" | "category" | "users";

export function ResourceDatabaseV2({ kind, locale, onScrapbook = () => undefined, active = true }: { kind: "abilities" | "items"; locale: Locale; onScrapbook?: (entry: Pokemon) => void; active?: boolean }) {
  const [query, setQuery] = useState("");
  const [categories, setCategories] = useState<string[]>([]);
