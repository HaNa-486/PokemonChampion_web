...
export function PokemonTableV2({ locale, format, onSelect }: { locale: Locale; format: "singles" | "doubles"; onSelect: (entry: Pokemon) => void }) {
  ...
  const matches = pokemon.filter((entry) => `${entry.name} ${entry.nameZh} ${entry.types.join(" ")}`.toLowerCase().includes(query.toLowerCase())
    && (!selectedTypes.length || selectedTypes.some((type) => entry.types.includes(type as Pokemon["types"][number])))
  ...
  const clearFilters = () => { setSelectedTypes([]); setForm("all"); setAbilityId(""); setKnownMoveIds([]); setStatMinimums({ hp: 0, attack: 0, defense: 0, specialAttack: 0, specialDefense: 0, speed: 0 }); setPage(1); };
  ...
  return <><section className="panel catalog-panel"><div className="panel-head"><div><p className="eyebrow">CURRENT REGULATION</p><h1>{copy.title}</h1><p>{matches.length} {copy.legal} · {copy.showing} {rangeStart}–{rangeEnd} {copy.of} {sorted.length} · {copy.hint}</p></div><label className="search-field"><span className="sr-only">Search Pokémon</span><input value={query} onChange={(event) => { setQuery(event.target.value); setPage(1); }} placeholder={copy.search} /></label></div>
    <div className="advanced-filters pokemon-advanced-filters">
      ...
      <div className="filter-group filter-select-row"><b>{locale === "zh-Hant" ? "特性" : "Ability"}</b><select aria-label="Filter by ability" value={abilityId} onChange={(event) => { setAbilityId(event.target.value); setPage(1); }}><option value="">{locale === "zh-Hant" ? "全部特性" : "All abilities"}</option>{abilities.map((entry) => <option key={entry.id} value={entry.id}>{localName(entry, locale)}</option>)}</select></div>
      <div className="filter-group filter-select-row"><b>{locale === "zh-Hant" ? "可學招式" : "Known moves"}</b><div><select aria-label="Add known move filter" value="" onChange={(event) => { if (event.target.value) setKnownMoveIds((current) => current.includes(event.target.value) ? current : [...current, event.target.value]); setPage(1); }}><option value="">{locale === "zh-Hant" ? "加入招式條件…" : "Add a move…"}</option>{moves.map((entry) => <option key={entry.id} value={entry.id}>{localName(entry, locale)}</option>)}</select>{knownMoveIds.map((id) => <button key={id} className="selected-filter" onClick={() => { setKnownMoveIds((current) => current.filter((entry) => entry !== id)); setPage(1); }}>{localName(moveById.get(id)!, locale)} ×</button>)}</div></div>
