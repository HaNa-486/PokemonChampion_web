...
function BattlePanel({ usage, locale }: { usage: BattleUsage | null | undefined; locale: Locale }) {
  ...
  const groups = categoryOrder.map((category) => ({ category, rows: usage.rows.filter((row) => row.category === category).sort((left, right) => left.rank - right.rank) })).filter((group) => group.rows.length);
