...
function LearnableSortHeader({ label, column, sort, onChange, initialDirection = "asc" }: { label: string; column: LearnableMoveSortKey; sort: LearnableMoveSort | null; onChange: (sort: LearnableMoveSort) => void; initialDirection?: SortDirection }) {
  const active = sort?.key === column;
  const nextDirection = active ? (sort.direction === "asc" ? "desc" : "asc") : initialDirection;
  return <th aria-sort={active ? (sort.direction === "asc" ? "ascending" : "descending") : "none"}><button className="sort-button" onClick={() => onChange({ key: column, direction: nextDirection })}>{label}<span aria-hidden="true">{active ? (sort.direction === "asc" ? "↑" : "↓") : "↕"}</span></button></th>;
}

function LearnableMoveTable({ moves, locale, sort, onSort }: { moves: Move[]; locale: Locale; sort: LearnableMoveSort | null; onSort: (sort: LearnableMoveSort) => void }) {
