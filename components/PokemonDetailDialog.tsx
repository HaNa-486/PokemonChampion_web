...
export function PokemonDetailPanel({ pokemon, locale, initialFormat, onClose, onBuild, onScrapbook, inline = false, buildLabel, visibleBattleCategories: controlledBattleCategories, onVisibleBattleCategoriesChange }: { pokemon: Pokemon; locale: Locale; initialFormat: "singles" | "doubles"; onClose?: () => void; onBuild: () => void; onScrapbook?: () => void; inline?: boolean; buildLabel?: string; visibleBattleCategories?: string[]; onVisibleBattleCategoriesChange?: (categories: string[]) => void }) {
  ...
  return <><div className="detail-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose?.(); }}><section className="pokemon-detail" role="dialog" aria-modal="true" aria-labelledby={titleId} onKeyDown={(event) => { if (event.key === "Escape" && !reverseSelection) onClose?.(); }}><button className="close-button" onClick={onClose} aria-label={copy.close}>×</button>{content}</section></div>{reverseDialog}</>;
}
