...
import { abilityById, abilityIdByUsageName, itemById, itemIdByUsageName, megaBasePokemonIdByStoneId, megaStoneIdByPokemonId, moveById, moveIdByUsageName, pokemon as catalogPokemon, pokemonById, usageEntityKey } from "../lib/catalog";
...
export function PokemonDetailPanel({ pokemon, locale, initialFormat, onClose, onBuild, onScrapbook, inline = false }: { pokemon: Pokemon; locale: Locale; initialFormat: "singles" | "doubles"; onClose?: () => void; onBuild: () => void; onScrapbook?: () => void; inline?: boolean }) {
  const titleId = useId();
  ...
  const [visibleBattleCategories, setVisibleBattleCategories] = useState<string[]>(categoryOrder);
  ...
  const moveEntries = useMemo(() => learnableMoves.filter((move) => {
    ...
    if (!inline) return compareLearnableMoves(left, right);
    return compareScrapbookLearnableMoves(pokemon.types, left, right);
  }), [inline, learnableMoves, moveQuery, priorities, types, categories, targets, properties, pokemon.types]);
