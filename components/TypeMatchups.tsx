...
function MatchupValues({ values }: { values: TypeMatchup[] }) {
  return <div>{values.length ? values.map((entry) => <span className="matchup-badge" key={entry.type}><TypeBadge type={entry.type} /><b>{formatMultiplier(entry.multiplier)}</b></span>) : <span className="matchup-none">—</span>}</div>;
}
