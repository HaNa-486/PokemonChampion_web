...
import { apiError, isSameOriginRequest, readJsonBody } from "../../../../../lib/api";
...
export async function POST(request: Request) {
  const auth = await authorize();
  if ("error" in auth) return auth.error;
  if (!isSameOriginRequest(request)) return apiError(403, "CROSS_ORIGIN_REQUEST", "Cross-origin write requests are not allowed.");
