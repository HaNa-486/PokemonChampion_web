...
describe("scrapbook user journey", () => {
  ...
  it("uses the sortable move-database columns for inline learnable moves without the properties column", async () => {
    ...
    const moveOrder = () => [...table.querySelectorAll("tbody tr")].map((row) => row.children[0]?.textContent);
    const defaultOrder = moveOrder();
    const powerSort = () => within(table as HTMLElement).getByRole("button", { name: /^Power/ });
    await user.click(powerSort());
    ...
    await user.click(powerSort());
    expect(within(table as HTMLElement).getByRole("columnheader", { name: /Power/ })).toHaveAttribute("aria-sort", "ascending");
    await user.click(powerSort());
    expect(within(table as HTMLElement).getByRole("columnheader", { name: /Power/ })).toHaveAttribute("aria-sort", "none");
    expect(moveOrder()).toEqual(defaultOrder);
    await user.click(within(table as HTMLElement).getAllByRole("button", { name: /^View \d+ Pokémon that can use/ })[0]);
