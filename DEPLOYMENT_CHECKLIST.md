...
- [ ] Pokémon DB detail and inline scrapbook detail use a height-bounded Move DB-style learnable-move table with sortable Move/Type/Class/Power/Accuracy/PP/Priority/Target/Effect/usable-Pokémon-count columns; numeric columns first sort descending, Properties and its filter remain Move-DB-only, and the scrapbook default remains own-type-first/canonical-type/Physical-Special-Status.
